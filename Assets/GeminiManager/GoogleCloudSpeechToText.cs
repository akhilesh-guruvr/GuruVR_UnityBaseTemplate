...
namespace GoogleSpeechToText.Scripts
{
    public class GoogleCloudSpeechToText : MonoBehaviour
    {
        ...
        // Added 'string languageCode' to the parameters
        public static void SendSpeechToTextRequest(byte[] bytes, string apiKey, string languageCode, Action<string> onSuccess, Action<BadRequestData> onError)
        {
            string base64Content = Convert.ToBase64String(bytes);

            // Safety fallback: if languageCode is empty, default to English
            ...
            var requestData = new SpeechToTextRequest
            {
                ...
                {
                    encoding = "LINEAR16",
                    // sampleRateHertz = 44100, // Make sure this matches your Microphone.Start frequency
                    languageCode = languageCode, // <--- UPDATED HERE
                    enableWordTimeOffsets = false
                },
                audio = new AudioData
                {
